...
export class ShareReceiverService {
  ...
  async receive(shareUrl: string, options: ShareReceiveOptions): Promise<ReceivedShare> {
    ...
    return new ReceivedShareImpl(identity, inspection.metadata.shareId, inspection.metadata, envelope, credentials, sign, options, this.fetchFn, this.config.credentialDiscoveryUrl ?? DEFAULT_CREDENTIAL_DISCOVERY, trustedNode);
  }
}
