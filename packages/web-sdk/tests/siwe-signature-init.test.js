    ...
    for (const extension of this.extensions) {
      if (extension.namespace && extension.defaultActions) {
        try {
          ...
          mockSessionManager.addDefaultActions(extension.namespace, defaults);
        } catch (error) {
...
describe('TinyCloudWeb UserAuthorization SIWE Sign-in with Signature', () => {
  ...
  describe('userAuthorization.generateSiweMessage()', () => {
    ...
    it('should generate a SIWE message with partial override', async () => {
      ...
      await tcw.userAuthorization.generateSiweMessage(address, partialSiweMessage);
    ...
    it('should apply extension capabilities during message generation', async () => {
      ...
      expect(pendingSession.sessionManager.addDefaultActions).toHaveBeenCalledWith(
        'test-extension', 
        ['action1', 'action2']
      );
      expect(pendingSession.sessionManager.addTargetedActions).toHaveBeenCalledWith(
        'test-extension', 
        'target1', 
        ['targetedAction1']
      );
    ...
    it('should handle extension capability errors gracefully', async () => {
      ...
      const faultyExtension = {
        ...
        defaultActions: jest.fn().mockRejectedValue(new Error('Default actions failed')),
        targetedActions: jest.fn().mockRejectedValue(new Error('Targeted actions failed')),
      };
    ...
    it('should reject invalid Ethereum addresses', async () => {
      ...
      for (const invalidAddress of invalidAddresses) {
        await expect(tcw.userAuthorization.generateSiweMessage(invalidAddress))
          .rejects
          .toThrow('Invalid Ethereum address format. Address must be a valid 0x-prefixed hex string.');
      }
    });
  ...
  describe('userAuthorization.signInWithSignature()', () => {
    ...
    it('should handle afterSignIn hook errors gracefully', async () => {
      const faultyExtension = {
        namespace: 'faulty-extension',
        afterSignIn: jest.fn().mockRejectedValue(new Error('AfterSignIn failed')),
      };
  ...
  describe('Extension integration', () => {
    it('should handle extensions without defaultActions or targetedActions', async () => {
      ...
      await expect(tcw.userAuthorization.generateSiweMessage(address)).resolves.toBeDefined();
    });
  ...
  describe('Error scenarios and edge cases', () => {
    ...
    it('should validate basic input parameters correctly', async () => {
      ...
      const result = await tcw.userAuthorization.generateSiweMessage(validAddress);
